...
:gotAdmin
    pushd "%CD%"
    CD /D "%~dp0"
    echo "Detecting filebrowser.exe..."
    if not exist "%~dp0filebrowser.exe" (
        echo "Ready to extract Filebrowser..."
        goto extractFilebrowser
    ) else (
...
:extractFilebrowser
    ...
    if not exist "%~dp0filebrowser.7z" (
            ...
            echo "Extracting filebrowser.7z, Please wait..."
